...
`main.ts` installs Pinia, then calls `useAuthStore().hydrate()` and only **after** it resolves installs the router (`app.use(router)`) and mounts. This order matters: installing the router kicks off its initial navigation (and `beforeEach` guards) immediately, independent of `app.mount()` timing — so if `app.use(router)` ran before `hydrate()` resolved, a hard reload on a role-gated route (e.g. `/admin/*`) would see `auth.user` still `null`, fail the role check, and get redirected to `/dashboard` before the real user data ever loaded. (This was an actual bug caught while building the admin users page — reloading `/admin/users` bounced straight to the dashboard until the fix.) `App.vue` is just `<router-view />`.
...
- Admin: a nested `admin` route under the same authenticated layout, component `layouts/AdminLayout.vue`, gated with `meta.roles: [Role.ADMIN]` (the first real use of the `meta.roles` mechanism that was set up in Epic 3 ahead of time). Its own children are Epic 6's individual admin pages — `admin-trigger-event` and `admin-users` so far, with a redirect from the bare `admin` path to the first one. `AdminLayout.vue` renders a left sidebar — `w-56 shrink-0`, its own `border`/`bg-surface` so it reads as a distinct panel against the page background, one link per admin page from a `navItems` array — + a content column with `<router-view />`, so each new admin page (notification log, alert management) is one new child route + one new `navItems` entry, no rework of the shell.

`AuthenticatedLayout.vue`'s `<main>` has no width constraint of its own (just padding) — this is deliberate so the sidebar can sit flush against the left edge and admin content can use the full remaining width. Pages that want a narrower reading column own that constraint themselves: `DashboardPage.vue` wraps its content in `mx-auto max-w-3xl`, and `LoginPage`/`SignupPage` already did the same with `max-w-sm`. `AdminUsersPage`/`AdminTriggerEventPage` intentionally don't cap width, since a data table benefits from the extra room.
...
- `admin-users.store.ts` — admin-only; `fetchUsers()`/`updateUser(id, input)`, the latter `PATCH`ing `/api/admin/users/:id` and replacing the matching entry in the local `users` array (same update-in-place pattern as `alerts.store.ts`).
...
- `components/ui/` — base building blocks (`BaseButton`, `BaseInput`, `BaseSelect`, `BaseCard`), reused everywhere forms/lists appear. `BaseSelect` supports a `disabled` prop (added for the admin users page's "can't edit your own row" case). New UI needs should extend or reuse these before adding new one-off elements (`.claude/rules/vue-patterns.md`).
...
- `components/admin/` — `AdminUserList`/`AdminUserListItem` (same list/list-item split as the alerts components, but a different edit UX by design): unlike `AlertListItem`, which fires an API call on every change, each `AdminUserListItem` holds its role/active edits in local refs and only emits `save` when its own "Save" button is clicked — that button is disabled until the local values actually differ from the `user` prop (a `computed` diff), and a `watch` on the `user` prop resyncs local state after a successful save (or an external refetch) so the button correctly goes back to disabled. Each row's controls are further disabled with a "(you)" note when the row is the logged-in admin's own account, mirroring the backend's self-modification guard client-side instead of just letting the request 400.
- `pages/` — orchestration only: `LoginPage`/`SignupPage` wire the auth store to a form; `DashboardPage` wires `alerts.store.ts` + `categories.store.ts` and renders the alert components; `AdminTriggerEventPage` wires `admin-events.store.ts` + `categories.store.ts`, client-side validates via `triggerMockEventSchema.safeParse`, and renders the last trigger's event + notifications inline; `AdminUsersPage` wires `admin-users.store.ts` + `auth.store.ts` (to know which row is "you") and renders `AdminUserList`. None make direct API calls of their own — that's always the stores' job.
