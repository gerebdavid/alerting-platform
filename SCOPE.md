...
| 6. Admin views | In progress | Being built as 4 separate pieces, in order: (1) **Trigger-mock-event UI — Done.** Established the admin route/nav shell everything else reuses: `router.ts` adds a `meta.roles: [Role.ADMIN]`-gated nested `admin` route under the authenticated layout (first real use of the role-gating built into the router in Epic 3); `AuthenticatedLayout.vue` gets a toggle nav link ("Admin" / "← Dashboard" depending on which side you're on, shown only when `auth.isAdmin`). `stores/admin-events.store.ts` (`triggerEvent`, holds only the latest result) + `pages/AdminTriggerEventPage.vue` call the existing `POST /api/admin/events/trigger` — no backend changes needed. (2) **User management — Done.** New backend module `packages/backend/src/modules/users/`: `GET /api/admin/users` (list all), `PATCH /api/admin/users/:id` (update `role`/`isActive`, blocked with 400 if an admin targets their own account — self-lockout guard), using a new `adminUserUpdateSchema` in `packages/shared` and the previously-unused `AdminUserUpdateRequest`/`UserResponse` DTOs. `layouts/AdminLayout.vue` was redesigned from a top tab-bar to a left sidebar (scales better with 2+ admin pages). `stores/admin-users.store.ts` + `pages/AdminUsersPage.vue` + `components/admin/AdminUserList(Item).vue` (role select + active checkbox per row, disabled with a "(you)" note for the admin's own row, mirroring the backend guard). Caught and fixed a real bug while verifying: a hard reload on a role-gated route (`/admin/*`) incorrectly bounced to the dashboard because `main.ts` installed the router before `auth.hydrate()` resolved, so the initial navigation's role check ran against a still-`null` user — fixed by deferring `app.use(router)` until after hydration (see ARCHITECTURE.md). Verified via `npm run build` (both packages), the full Bruno collection (new `admin/ListUsers`/`admin/UpdateUser`), curl checks (403 non-admin, 401 no token, 400 self-modification, 404 unknown user), and a scripted Playwright walkthrough (role/active changes persist across a reload, own-row controls disabled, 0 console errors). (3) Notification log, (4) alert management — not started. |
